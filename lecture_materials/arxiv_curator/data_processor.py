...
class DataProcessor:
    ...
    def _get_range_start(self) -> str:
        ...
        """
        fallback = time.strftime("%Y%m%d%H%M", time.gmtime(time.time() - 24 * 3600 * 3))

        if not self.spark.catalog.tableExists(self.papers_table):
            logger.info(f"No existing arxiv_papers table. Starting from 3 days ago: {fallback}")
            return fallback

        row = self.spark.sql(f"""
            SELECT max(processed) AS max_processed
            FROM {self.papers_table}
        """).collect()[0]

        max_processed = row["max_processed"]

        if max_processed is None:
            logger.info(
                f"Existing arxiv_papers table has no processed values. "
                f"Starting from 3 days ago: {fallback}"
            )
            return fallback
